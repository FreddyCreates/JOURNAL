/**
 * SCL Parser - Scaling configurations
 */
export class SCLParser {
  ...
  parse(source) {
    ...
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'SCALE')) items.push(this.parseSCALE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'SCL', version: '1.0.0' };
  }
  parseSCALE() {
    this.expect('KEYWORD', 'SCALE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'SCALE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['SCALE', 'VERSION'];
    let i = 0;

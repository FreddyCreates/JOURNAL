/**
 * PIL Parser - Subconscious patterns
 */
export class PILParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'PATTERN')) items.push(this.parsePATTERN());
      else this.advance();
    }
    return { type: 'Program', items, language: 'PIL', version: '1.0.0' };
  }
  parsePATTERN() {
    this.expect('KEYWORD', 'PATTERN');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'PATTERN', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['PATTERN', 'VERSION'];
    let i = 0;

/**
 * ISL Parser - Knowledge synthesis
 */
export class ISLParser {
  ...
  parse(source) {
    ...
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'INSIGHT')) items.push(this.parseINSIGHT());
      else this.advance();
    }
    return { type: 'Program', items, language: 'ISL', version: '1.0.0' };
  }
  parseINSIGHT() {
    this.expect('KEYWORD', 'INSIGHT');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'INSIGHT', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['INSIGHT', 'VERSION'];
    let i = 0;

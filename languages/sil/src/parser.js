/**
 * SIL Parser - Identity management
 */
export class SILParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'IDENTITY')) items.push(this.parseIDENTITY());
      else this.advance();
    }
    return { type: 'Program', items, language: 'SIL', version: '1.0.0' };
  }
  parseIDENTITY() {
    this.expect('KEYWORD', 'IDENTITY');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'IDENTITY', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['IDENTITY', 'VERSION'];
    let i = 0;

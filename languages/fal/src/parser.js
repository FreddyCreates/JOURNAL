/**
 * FAL Parser - Feedback analysis
 */
export class FALParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'FEEDBACK')) items.push(this.parseFEEDBACK());
      else this.advance();
    }
    return { type: 'Program', items, language: 'FAL', version: '1.0.0' };
  }
  parseFEEDBACK() {
    this.expect('KEYWORD', 'FEEDBACK');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'FEEDBACK', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['FEEDBACK', 'VERSION'];
    let i = 0;

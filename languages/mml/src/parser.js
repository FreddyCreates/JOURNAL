...
export class MMLParser {
  ...
  parse(source) {
    ...
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'MIGRATION')) items.push(this.parseMIGRATION());
      else this.advance();
    }
    return { type: 'Program', items, language: 'MML', version: '1.0.0' };
  }
  parseMIGRATION() {
    this.expect('KEYWORD', 'MIGRATION');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'MIGRATION', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['MIGRATION', 'VERSION'];
    let i = 0;

/**
 * FRL Parser - Failure recovery
 */
export class FRLParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'FAILURE')) items.push(this.parseFAILURE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'FRL', version: '1.0.0' };
  }
  parseFAILURE() {
    this.expect('KEYWORD', 'FAILURE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'FAILURE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['FAILURE', 'VERSION'];
    let i = 0;

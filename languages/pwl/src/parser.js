/**
 * PWL Parser - Practical wisdom
 */
export class PWLParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'PRACTICE')) items.push(this.parsePRACTICE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'PWL', version: '1.0.0' };
  }
  parsePRACTICE() {
    this.expect('KEYWORD', 'PRACTICE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'PRACTICE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['PRACTICE', 'VERSION'];
    let i = 0;

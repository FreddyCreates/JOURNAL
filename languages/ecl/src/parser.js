/**
 * ECL Parser - Economic contracts
 */
export class ECLParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'ECONOMIC')) items.push(this.parseECONOMIC());
      else this.advance();
    }
    return { type: 'Program', items, language: 'ECL', version: '1.0.0' };
  }
  parseECONOMIC() {
    this.expect('KEYWORD', 'ECONOMIC');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'ECONOMIC', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['ECONOMIC', 'VERSION'];
    let i = 0;

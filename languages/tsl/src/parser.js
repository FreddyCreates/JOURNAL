/**
 * TSL Parser - Teaching strategies
 */
export class TSLParser {
  ...
  parse(source) {
    ...
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'STRATEGY')) items.push(this.parseSTRATEGY());
      else this.advance();
    }
    return { type: 'Program', items, language: 'TSL', version: '1.0.0' };
  }
  parseSTRATEGY() {
    this.expect('KEYWORD', 'STRATEGY');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'STRATEGY', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['STRATEGY', 'VERSION'];
    let i = 0;

/**
 * EDL Parser - Educational courses
 */
export class EDLParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'COURSE')) items.push(this.parseCOURSE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'EDL', version: '1.0.0' };
  }
  parseCOURSE() {
    this.expect('KEYWORD', 'COURSE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'COURSE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['COURSE', 'VERSION'];
    let i = 0;

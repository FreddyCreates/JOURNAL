/**
 * UEL Parser - User experience
 */
export class UELParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'EXPERIENCE')) items.push(this.parseEXPERIENCE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'UEL', version: '1.0.0' };
  }
  parseEXPERIENCE() {
    this.expect('KEYWORD', 'EXPERIENCE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'EXPERIENCE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['EXPERIENCE', 'VERSION'];
    let i = 0;

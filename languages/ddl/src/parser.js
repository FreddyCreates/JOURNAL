...
export class DDLParser {
  ...
  parse(source) {
    ...
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'DATA')) items.push(this.parseDATA());
      else this.advance();
    }
    return { type: 'Program', items, language: 'DDL', version: '1.0.0' };
  }
  parseDATA() {
    this.expect('KEYWORD', 'DATA');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'DATA', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['DATA', 'VERSION'];
    let i = 0;

/**
 * TIL Parser - Temporal reasoning
 */
export class TILParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'TIMELINE')) items.push(this.parseTIMELINE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'TIL', version: '1.0.0' };
  }
  parseTIMELINE() {
    this.expect('KEYWORD', 'TIMELINE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'TIMELINE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['TIMELINE', 'VERSION'];
    let i = 0;

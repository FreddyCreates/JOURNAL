/**
 * ERR Parser - Error handling
 */
export class ERRParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'ERROR')) items.push(this.parseERROR());
      else this.advance();
    }
    return { type: 'Program', items, language: 'ERR', version: '1.0.0' };
  }
  parseERROR() {
    this.expect('KEYWORD', 'ERROR');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'ERROR', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['ERROR', 'VERSION'];
    let i = 0;

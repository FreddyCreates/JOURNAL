/**
 * IIL Parser - System interfaces
 */
export class IILParser {
  constructor() { this.tokens = []; this.position = 0; }
  parse(source) {
    this.tokens = this.tokenize(source); this.position = 0;
    const items = [];
    while (!this.isAtEnd()) {
      if (this.check('KEYWORD', 'INTERFACE')) items.push(this.parseINTERFACE());
      else this.advance();
    }
    return { type: 'Program', items, language: 'IIL', version: '1.0.0' };
  }
  parseINTERFACE() {
    this.expect('KEYWORD', 'INTERFACE');
    const name = this.expect('IDENTIFIER').value;
    this.expect('LBRACE');
    const item = { type: 'INTERFACE', name, metadata: {} };
  ...
  tokenize(source) {
    const tokens = [], keywords = ['INTERFACE', 'VERSION'];
    let i = 0;
